...
/* ✅ Single constant width for ALL cards — desktop */
const CARD_WIDTH = 260;
/* Mobile card width */
const CARD_WIDTH_MOBILE = 220;

function GlassCard({ style = {}, mobileStyle = {}, children }) {
  /* We use a data attribute to swap styles via CSS custom properties */
  return (
    <>
      <div
...
export function CodeCard() {
  return (
    <>
      <style>{`
        .floating-card-desktop { display: block; }
        .floating-card-mobile  { display: none;  }
        @media (max-width: 767px) {
          .floating-card-desktop { display: none !important; }
          .floating-card-mobile  { display: block !important; }
        }
      `}</style>
      {/* Desktop */}
      <div
      ...
      >
        <CodeCardContent mobile />
      </div>
    </>
  );
}

function CodeCardContent({ mobile }) {
  const fs = mobile ? 9 : 10;
  const fsCode = mobile ? 9.5 : 10.5;
  const pad = mobile ? '10px 12px' : '14px 16px';
  return (
    <div style={{ padding: pad }}>
      <p style={{ fontFamily: 'monospace', fontSize: fs, color: 'rgba(196,181,253,0.9)', marginBottom: 6 }}>
        &lt;code&gt;
      </p>
      <pre style={{
        fontFamily: 'monospace', fontSize: fsCode, lineHeight: 1.7,
...
<span style={{color:'#c4b5fd'}}>const</span> <span style={{color:'#fff'}}>developer</span> = {`{
  ...
  `}]{`
      ...
      <p style={{ fontFamily: 'monospace', fontSize: fs, color: 'rgba(196,181,253,0.9)', marginTop: 6 }}>
        &lt;/code&gt;
      </p>
...
export function TechCard() {
  return (
    <>
      ...
      {/* Mobile — positioned below code card */}
      <div
        className="floating-card-mobile"
        style={{
          position: 'absolute',
          ...
          boxShadow: '0 8px 32px rgba(0,0,0,0.18)',
          zIndex: 10,
          animation: 'floatYSlowMobile 5s ease-in-out infinite',
          animationDelay: '0.6s',
        }}
      >
        <TechCardContent mobile />
      </div>
    </>
  );
}

function TechCardContent({ mobile }) {
  const pad = mobile ? '10px 12px' : '14px 16px';
  const iconSize = mobile ? 34 : 40;
  const lucideSize = mobile ? 15 : 18;
  return (
    <div style={{ padding: pad }}>
      <p style={{ color: '#fff', fontWeight: 700, fontSize: mobile ? 11 : 13, marginBottom: 10, fontFamily: "'Syne', sans-serif" }}>
        Tech I Use
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: mobile ? 6 : 8 }}>
        {techIcons.map(({ Icon, color, label, bg }) => (
          ...
          >
            <Icon size={lucideSize} color={color} />
          </div>
        ))}
      </div>
    </div>
...
export function AboutCard({ onNavigate }) {
  return (
    <>
      ...
      >
        <AboutCardContent onNavigate={onNavigate} />
      </div>
      {/* Mobile — at bottom */}
      <div
        className="floating-card-mobile"
      ...
      >
        <AboutCardContent onNavigate={onNavigate} mobile />
      </div>
    </>
  );
}

function AboutCardContent({ onNavigate, mobile }) {
  const pad = mobile ? '10px 12px' : '14px 16px';
  return (
    <div style={{ padding: pad }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <p style={{ color: '#fff', fontWeight: 700, fontSize: mobile ? 11 : 13, fontFamily: "'Syne', sans-serif" }}>
          About Me
        </p>
      </div>
      ...
      <button
        ...
        style={{
          ...
          fontSize: mobile ? 10.5 : 11.5, fontWeight: 700,
